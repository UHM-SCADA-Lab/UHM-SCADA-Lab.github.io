...
const VLAN = () => (
  <span>
    ...
    <p>VLANS are important to our lab. It gives us the ability to control and restrict which devices can communicate with each other. In this lab, we have four VLANs. The first VLAN is the general VLAN which is where most of our Pi’s are connected to, the second is the SDN controller, the third is the SEL[3] (Schweitzer Engineering Laboratories), and the fourth is the security VLAN. The reason the Pi’s are grouped to a specific VLAN is because we want the Pi we are working with to be in the same network as the devices that it needs to communicate with. By separating the Pi&apos;s into their own VLAN, we can ensure that their traffic is isolated from other devices on the network.</p>
